 ...
 *
 * Links @keyring into the session keyring and reads
 * the payload of the key specified by @key_id.
 * @len holds the size of the returned buffer.
 * If @keyring is NULL the default keyring '.nvme'
 * is used.
 *
 ...
 * @key_data:	TLS key in PSK interchange format
